...
class Recognizer3D(BaseRecognizer):
    ...
    def get_labels(self, file_name, id):
    ...
    def forward_train(self, imgs, labels, **kwargs):
        ...
        x, kloss = self.extract_feat(imgs)
    ...
    def _do_test(self, imgs):
        ...
        if self.max_testing_views is not None:
            ...
            while view_ptr < total_views:
                ...
                x, kloss = self.extract_feat(batch_imgs)
                if self.with_neck:
                    x, _ = self.neck(x)
                feats.append(x)
            ...
            feat, kloss = self.extract_feat(imgs)
            if self.with_neck:
