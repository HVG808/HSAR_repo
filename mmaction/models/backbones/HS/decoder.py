...
class ASDecoder(nn.Module):
    def __init__(self, num_layers, r1, r2, num_f_maps, input_dim, num_classes, att_type, alpha):
        super(ASDecoder, self).__init__()#         self.position_en = PositionalEncoding(d_model=num_f_maps)
    ...
    def forward(self, x, fencoder, mask):
        ...
        fencoder = self.conv_1x1(fencoder)
...
def build_ASDecoder(num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes):
    return nn.ModuleList([copy.deepcopy(ASDecoder(num_layers, r1, r2, num_f_maps, input_dim, num_classes, att_type='sliding_att', alpha=exponential_descrease(s))) for s in range(num_decoders)])